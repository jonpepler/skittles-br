...
const empty: Buckets = { onSign: [], onEvent: [], onReceive: [] }
...
describe('summarise', () => {
  ...
  it('groups by trigger, ordering sign → event → receive', () => {
    ...
    const result = summarise({ onSign: [t], onEvent: [t], onReceive: [t] })
    expect(result.map((s) => s.trigger)).toEqual(['onSign', 'onEvent', 'onReceive'])
  })
})
